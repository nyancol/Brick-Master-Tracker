...
This spec defines the manual theme toggle system that allows users to override the automatic `prefers-color-scheme` detection and manually select between Manuscript (light), Scriptorium (dark), or System-follow modes, with persistent preference storage.
