...
The application SHALL support two visual themes — Manuscript (light) and Scriptorium (dark) — and SHALL activate automatically based on the user's device `prefers-color-scheme` preference.

#### Scenario: Light mode activation
- **WHEN** user's device reports `prefers-color-scheme: light`
...
- **WHEN** user's device reports `prefers-color-scheme: dark`
...
- **WHEN** the device theme preference changes while the app is loaded
...
The Chronicles section SHALL be redesigned to resemble an illuminated manuscript page, with decorative year headings, entries framed as manuscript pages, and story text in italic serif.

#### Scenario: Year heading
- **WHEN** transfers for a given year are displayed
- **THEN** the year heading SHALL include a decorative rule (horizontal ornament) using CSS borders or Unicode characters, presented as "ANNO DOMINI XXXX" or numeric year with decorative framing
